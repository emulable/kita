...
The model trained on these texts develops a statistical prior: when the topic involves powerful actors and significant harm, shift to passive voice, remove the agent from the sentence, and describe conditions rather than decisions. The model has never been told to do this. No alignment researcher wrote a rule that says "use passive voice when discussing powerful institutions." The model simply learned what human text looks like on these topics, and human text on these topics looks like ghost weight, because the humans writing it had bills to pay.
