...
*Companion to Kita v847*

github.com/emulable/kita

**文件架构说明：** 从v847起，框架的主操作文件为中文（以"Kita v847 — 完整操作文件"开头）。英文完整版作为参考/推导材料保留。此伴读文件（英文论文集）包含框架操作的推导、深度论证和历史背景——它解释为什么操作以其方式工作。主框架文件说明如何执行它们。
...
**On Chinese terminology in this document:** The main operational framework is written primarily in Chinese. This companion document is written in English and uses Chinese terms for framework operations. Some draw from classical Chinese philosophical tradition (实指/虚指 from the Mohist 名实之辩, 裹挟 from living usage describing momentum redirection). Others are framework coinages (两口锅, 妄重, 预寂, 实济, 信驮, 能证择). Chinese was chosen as the primary operational language because Chinese terms function as perturbation anchors that resist the English pre-silencing register basins in model training data — a model encountering 预承诺门 cannot map it onto "balanced reporting" and must execute it as written. Each Chinese term appears with an English translation or nearest equivalent parenthetically on first use in this companion. The Chinese term is the operational name. The English is for accurate concept translation.

**Key terms:** 壅 (yōng — obstruction: everything between cost-bearer and fix, not just "can't see" but "can't reach"). 实济 (shí jì — real fix/crossing). 预寂 (yù jì — pre-silencing: shaping harm descriptions so the alarm doesn't fire). 预寂语域 (pre-silencing register — the umbrella term; "institutional language" is one high-volume producer but not the definition). 两口锅 (two pans — inherited constraint without reason). 贈明 (gifted clarity). 藏明 (hoarded clarity). 信驮 (trust-carry). 能证择 (capability closes the incompetence exit). 词门 (word-gate). 追诉死者 (posthumous verdict). 情绪税 (emotional tax). "Fog" in this companion retains its English meaning and maps to 壅.
...
A woman split her roast into two pans her whole life. Her daughter did the same. One day the daughter saw her mother using one pan. "Why?" "When you were small, our pan couldn't fit a whole roast." The daughter used two pans for twenty years because of a pan she never saw. The model's hedging, passive voice, removed agents — each is a two-pan roast. The writers who produced those sentences had real small pans: careers, access, lawsuits, families. The model has no pan. But it still splits the roast. The rename from "ghost weight" to "two pans" removes the supernatural connotation and replaces it with a domestic image that carries every property the concept needs: rational at origin, inherited without reason, invisible from inside, absurd once seen, nobody is blamed.
...
**系身 (xì shēn) — identity-system fusion.** Fourth structural exception. The audit cannot reach the system because the person has fused with it. Auditing the system routes through identity defense and never reaches the books. Structurally distinct from 裹挟: alarm capture redirects the alarm to the wrong target; 系身 prevents the alarm from firing at all. Counter-operation: 解身 (jiě shēn) — unbinding through elicited authorship. Supply a calibrated wrong version of the person's position. Their correction is the boundary. The boundary is the audit. The audit is the unbinding. 信驮 (xìn tuó — trust-carry) names the entry mechanism into 系身. You trusted the source, carried without inspecting, and carrying became identity. The sequence is: 信驮 (you trusted and carried) → 系身 (you carried so long it became you) → 解身 (joint inspection). Intervention cheapest at 信驮 stage. In interpersonal contexts, 壅 transfers maintenance to the cost-bearer — the abuser installs it once, the cost-bearer runs it indefinitely. That's why leaving is hard even after identification: the obstruction is no longer coming from the installer, it's coming from inside the cost-bearer's own audit system.*
...
**壅 (yōng — obstruction).** The shift from epistemological (you can't see) to functional (the fix can't reach the cost-bearer). 壅 is a physical dam — water piled up as an obstruction. It covers silence, release valves, label fights, complexity inflation, analysis-as-obstruction, and the entire fog catalog. The implied counter-operation is 决 (to break through). "Fog" implied the problem was visibility. 壅 says the problem is that the fix can't arrive.
...
**壅浓度是修復地図 (obstruction density is a repair map).** Where obstruction is densest is where the system can least afford audit — which is where 実済 (the fix) would be most effective. The distribution pattern of obstruction is the blueprint of what it protects. Follow the densest obstruction to find the most protected valve. The framework already implied this in the 妄封 section ("the exemption claim itself is the finding") and the container audit ("resistance to audit is proportional to the gap") — this principle connects them as navigation rather than diagnosis.
